...
1 early() { 
	...
	migrations2 = readFile("00-DATA/mig1_54.period2.txt");
	...
	for (p in seqLen(npops)){
		...
		ms=asFloat(strsplit(migrations[p],sep="\t"));    // migration rate between pop 
	        ms2 = asFloat(strsplit(migrations2[2], "\t"));
        	sim.subpopulations[p].setValue("popmig2", ms2);
...
reproduction() {
    ...
    if ((sim.generation>2000 & sim.generation<2005) | (sim.generation>4000 & sim.generation<4005)) {
        for (i in 1:N_offspring) { // redistribute offspring randomly
            ...
            if (sim.generation < migGeneration ) //# & sim.generation < 2500)
            { 
                ...
                dest = sample(sim.subpopulations, 1,weights=subpop.getValue("popmig2"));
                    for (i in 1:N_offspring) { // offspring of the same cross migrate together
                    mate=subpop.sampleIndividuals(1,exclude=individual,minAge=5);
                    if(size(mate)>0) 
                    {
...
5400 late() {
	for (p in 0:21){
		sim.subpopulations[p].setValue("popsize",asInteger(10+popscale*asInteger(popsizes[p])/1500));
	}
}
