...
slim <- read.table("02.results_manitou/run_2_mig_crash_south_g4800.reshaped.txt") %>%
...
dat <- read.table("01.data/54pop_balanced_designed_present_and_future_climate_manual_extraction.txt",T)
...
slim_before <- filter(slim, generation < 4800 & generation >=4700)
slim_connec <- filter(slim, generation < 5400 & generation >= 4800)
slim_crash <-  filter(slim, generation < 5500 & generation >= 5400)
slim_after <-  filter(slim, generation >= 5500)

#create folder architecture:
folder1 = "png_mig_and_crash_south_54_pop/before"
folder2 = "png_mig_and_crash_south_54_pop/connectivity_change"
folder3 = "png_mig_and_crash_south_54_pop/connectivity_change_and_bottleneck"
folder4 = "png_mig_and_crash_south_54_pop/warming_time"

system(paste0("mkdir -p ", folder1))
system(paste0("mkdir -p ", folder2))
system(paste0("mkdir -p ", folder3))
system(paste0("mkdir -p ", folder4))
...
### during crash:
temps <- unique(sort(slim_connec$generation))
df3 = filter(df2, x > -99.5 & x < -39.5 & y >29.5 & y < 75.5 )
for(gen in temps)
{
	p1 <- filter(slim_connec, generation == gen) #%>%
  	p2 <- ggplot() +  
  	geom_tile(data=df3, aes(x=x, y=y, fill=value), alpha=0.8) + 
  		lims(x = c(-99,-40), y = c(30,75)) +
  		scale_fill_viridis() + 	coord_equal() + theme_map() + #theme_bw()
  	geom_point(data = p1, mapping = aes(x=Lon, y=Lat, color=Fitness, shape=status), size = 2) +
	    scale_color_continuous(breaks =c(min(p1$Fitness),max(p1$Fitness)), 
				    labels = c("min", "max")) +
  	labs(title = "fitness during connectivity change", subtitle=paste0("generation: ", gen)) + 
  	theme(plot.title = element_text(hjust = 0.5, size = 14, face = "bold"),
	      plot.subtitle = element_text(hjust = 0.5, size = 12, face = "bold")) +
      	#guides(size = "none", colour = guide_colourbar(order = 1) )  + 
      	guides( colour = guide_colourbar(order = 1), shape = guide_legend(order = 2),
...
for(gen in temps)
{
	p1 <- filter(slim_crash, generation == gen) #%>%
  	...
  	geom_point(data = p1, mapping = aes(x=Lon, y=Lat, color=Fitness, shape=status), size = 2) +
	    scale_color_continuous(breaks =c(min(p1$Fitness),max(p1$Fitness)), 
				    labels = c("min", "max")) +
  	labs(title = "fitness during connectivity change and bottleneck", 
	       subtitle=paste0("generation: ", gen)) + 
  	theme(plot.title = element_text(hjust = 0.5, size = 14, face = "bold"),
	      plot.subtitle = element_text(hjust = 0.5, size = 12, face = "bold")) +
      	#guides(size = "none", colour = guide_colourbar(order = 1) )  + 
...
for(gen in temps)
{
	p1 <- filter(slim_after, generation == gen) #%>%
  	...
  	geom_point(data = p1, mapping = aes(x=Lon, y=Lat, color=Fitness,shape=status), size = 2) +
	    scale_color_continuous(breaks =c(min(p1$Fitness),max(p1$Fitness)), 
				   labels = c("min", "max")) +
  	labs(title = "fitness during climate warming, connectivity change & bottleneck",
	       subtitle=paste0("generation: ", gen)) + 
  	theme(plot.title = element_text(hjust = 0.5, size = 14, face = "bold"),
	      plot.subtitle = element_text(hjust = 0.5, size = 12, face = "bold")) +
	guides( colour = guide_colourbar(order = 1), shape = guide_legend(order = 2),
	...
	png(paste0(folder4,"/input",gen,".png"),width = 2480, height=1500, res = 300)
	print(p2)
	dev.off()
}
...
nom2 <- '54pop_during_connectivity.mp4'
nom3 <- '54pop_connectivity_and_bottleneck.mp4'
nom4 <- '54pop_connectivity_and_bottleneck_during_warming.mp4'

setwd(p1)
av::av_encode_video(list.files(p1, '*.png'), framerate = 18,    output = nom1)
#before warming:
setwd(p2)
av::av_encode_video(list.files(p2, '*.png'), framerate = 15,    output = nom2)
setwd(p3)
av::av_encode_video(list.files(p3, '*.png'), framerate = 06,    output = nom3)
setwd(p4)
av::av_encode_video(list.files(p4, '*.png'), framerate = 05,    output = nom4)
setwd(p)
...
write.table(c(z1,z2,z3,z4),"inputs.txt",quote=F,row.names=F,col.names=F)
#2. merge with ffmpeg:
