...
initialize() {
	...
	defineConstant("N_offspring",5);                // number of offspring, can try differents values
	initializeMutationRate(1e-05);                  // fast mutation rate for quick run
        
	// age-related mortality: we simulated a (very) long lived species 
        defineConstant("L", c(0.5,0.25, rep(0,60),0.25,0.75, 1));
	...
	initializeGenomicElementType("g1", m1, 1.0);			//genomic element for neutral mutation
	initializeGenomicElementType("g2", m2, 1.0);			//genomic element for thermal QTL
...
1 early() { 
   ...
   sim.chromosome.setMutationRate(1e-5*25);    //increase mutation rate again to speed up code
}
...
reproduction() {
	if(size(subpop.individuals)>1 & individual.age>2){
		// The migration is stopped here between  2000 & 2005 and 4000 & 4005 generations intervals
...
2: early() {
	...
	for (subpop in sim.subpopulations) { 
		...
		//density dependance below (fitness is adjust accordingly):
		ages = inds.age;
		mortality = L[ages];
...
// write output to table at each generation from 4500 to the end of global warming
// for adults in each pop: 
// - mean fitness
// - mean phenotype
